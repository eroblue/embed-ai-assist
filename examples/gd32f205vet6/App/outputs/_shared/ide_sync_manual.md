...
- 生成时间：2026-09-20T16:41:10
- 目标工程：App
- 失败原因：未发现 IDE 工程文件（布局 IDE 目录 GCC、IAR、MDK-ARM 下无 .uvprojx / .ewp）；IDE 工程由用户手动创建，请先创建工程后重新运行本工具
...
- `Drivers/BSP/Src/fsmc_init.c`
- `Drivers/BSP/Src/gpio_init.c`
- `Drivers/BSP/Src/hal_init.c`
- `Drivers/BSP/Src/nvic_init.c`
- `Drivers/BSP/Src/pwm_init.c`
- `Drivers/BSP/Src/spi_init.c`
...
- **工程文件不存在时**：请先在 IDE 中手动创建工程（选芯片、编译器等配置由人工确认更可靠），保存到 `<项目根>/<目标工程>/` 下的 IDE 工程目录（`GCC/`、`IAR/`、`MDK-ARM/`，目录结构见 docs/PROJECT_LAYOUT.md），再重新运行 `python skills/_shared/scripts/ide_sync.py --project <项目根> --target <App|BootLoader> --ide <keil|iar>` 即可自动同步（仅 Keil/IAR 支持自动同步，其他 IDE 按本清单手动处理）。
