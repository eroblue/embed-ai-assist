...
volatile bit nMCLRR               @ ((unsigned)&PCON*8)+3;
volatile bit IERRR                @ ((unsigned)&PCON*8)+4;
...
// bit and bitfield definitions
volatile bit PSRC20              @ ((unsigned)&PSRC2*8)+0;
volatile bit PSRC21              @ ((unsigned)&PSRC2*8)+1;
volatile bit PSRC22              @ ((unsigned)&PSRC2*8)+2;
volatile bit PSRC23              @ ((unsigned)&PSRC2*8)+3;
#ifndef _LIB_BUILD
volatile union {
    struct {
        unsigned	PSRC20                : 1;
...
volatile bit T1OC4CE                @ ((unsigned)&TIM1CCMR4*8)+7;
...
// bit and bitfield definitions
volatile bit DATAH                 @ ((unsigned)&URDATAH*8)+0;
#ifndef _LIB_BUILD
volatile union {
    struct {
        unsigned	DATAH            : 1;
...
// bit and bitfield definitions
// bit and bitfield definitions
volatile bit URRAR0                   @ ((unsigned)&URRAR*8)+0;
volatile bit URRAR1                   @ ((unsigned)&URRAR*8)+1;
volatile bit URRAR2                   @ ((unsigned)&URRAR*8)+2;
volatile bit URRAR3                   @ ((unsigned)&URRAR*8)+3;
#ifndef _LIB_BUILD
volatile union {
    struct {
        unsigned	URRAR0            : 1;
...
volatile bit CPOL                   @ ((unsigned)&URSYNCR*8)+1;
volatile bit CPHA                   @ ((unsigned)&URSYNCR*8)+2;
...
volatile bit LVDDEB                   @ ((unsigned)&LVDCON1*8)+3;
