...
{
    int32_t r = driver_lcd_init(); /* 内含 par_port init + 上电序列 + 清屏 */
...
void app_lcd_display_poll(void)
{
    ...
    /* E[渲染 4 行完整画面] → H[经 driver_lcd 写入 LCD 显存] */
    (void)disp_copy(s_line, "ENV MONITOR");
    r = driver_lcd_show_line(0u, s_line);
    ...
    if (r == PORT_OK) { r = driver_lcd_show_line(1u, s_line); }

    len = disp_copy(s_line, "L:");
    len += disp_append_int(s_line + len, env->light);
    (void)disp_copy(s_line + len, "%");
    if (r == PORT_OK) { r = driver_lcd_show_line(2u, s_line); }
    ...
    if (r == PORT_OK) { r = driver_lcd_show_line(3u, s_line); }
...
{
    uint16_t i = 0;
    while ((src[i] != (char)0) && (i < DRIVER_LCD_LINE_CHARS)) {
        dst[i] = src[i];
        i++;
    }
    dst[i] = (char)0;
...
static uint16_t disp_append_int(char *dst, int32_t value)
{
    ...
    while ((n > 0u) && (pos < DRIVER_LCD_LINE_CHARS)) {
        n--;
        dst[pos++] = tmp[n];
    }
    dst[pos] = (char)0;
