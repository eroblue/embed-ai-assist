...
- 生成时间：2026-09-19T19:32:13
- 目标工程：App
- 失败原因：未发现 IDE 工程文件（project_files/ 下无 .uvprojx / .ewp）；IDE 工程由用户手动创建，请先创建工程后重新运行本工具
...
### group：init
- `src/init/adc_init.c`
- `src/init/clock_init.c`
- `src/init/gpio_init.c`
- `src/init/hal_init.c`
- `src/init/i2c_init.c`
- `src/init/nvic_init.c`
- `src/init/spi_init.c`
...
- **工程文件不存在时**：请先在 IDE 中手动创建工程（选芯片、编译器等配置由人工确认更可靠），保存到 `<项目根>/<目标工程>/project_files/` 下，再重新运行 `python skills/_shared/scripts/ide_sync.py --project <项目根> --target <App|BootLoader> --ide <keil|iar>` 即可自动同步（仅 Keil/IAR 支持自动同步，其他 IDE 按本清单手动处理）。
